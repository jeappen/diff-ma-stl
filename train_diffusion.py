import orbax.checkpoint as ocp
from datetime import datetime
from flax.training import orbax_utils
from jax import config as jax_config
from orbax.checkpoint import PyTreeCheckpointer

from gcbfplus.diffusion.diffusion.diffusion import (
    make_train_step,
    create_denoiser_train_state,
)
from gcbfplus.diffusion.environments.dataset import load_dataset
from gcbfplus.diffusion.util import *


def make_train(args, dataset, val_dataset, num_epochs, env=None, rollout_fn=None, sample_rollout_fn=None):
    diffusion_train_fn = make_train_step(args, env, rollout_fn, sample_rollout_fn)

    def train(rng, denoiser_state, ema_denoiser_state):
        # --- TRAIN LOOP ---
        def _epoch_train_step(runner_state, _):
            denoiser_state, ema_denoiser_state, rng = runner_state

            def _batch_train_step(runner_state, batch):
                # --- Update model on batch ---
                denoiser_state, ema_denoiser_state, rng = runner_state
                rng, _rng = jax.random.split(rng)
                denoiser_state, loss = diffusion_train_fn(_rng, batch, denoiser_state)

                # --- Update EMA (exponential moving average) model ---
                new_params = ema_update(args, denoiser_state, ema_denoiser_state)
                ema_denoiser_state = ema_denoiser_state.replace(params=new_params)
                return (denoiser_state, ema_denoiser_state, rng), jnp.mean(loss)

            # --- Construct and iterate over dataset mini-batches ---
            rng, _rng = jax.random.split(rng)
            batched_dataset = shuffle_and_batch_dataset(_rng, dataset, args.batch_size)
            (denoiser_state, ema_denoiser_state, rng), batch_losses = jax.lax.scan(
                _batch_train_step,
                (denoiser_state, ema_denoiser_state, rng),
                batched_dataset,
            )
            return (denoiser_state, ema_denoiser_state, rng), jnp.mean(batch_losses)

        # --- Iterate over dataset epochs ---
        (denoiser_state, ema_denoiser_state, _), train_losses = jax.lax.scan(
            _epoch_train_step,
            (denoiser_state, ema_denoiser_state, rng),
            None,
            length=num_epochs,
        )

        # --- Compute validation loss ---
        def _batch_eval(rng, batch):
            rng, _rng = jax.random.split(rng)
            _, loss = diffusion_train_fn(_rng, batch, ema_denoiser_state)
            return rng, jnp.mean(loss)

        rng, _rng = jax.random.split(rng)
        batched_val_dataset = shuffle_and_batch_dataset(
            _rng, val_dataset, args.batch_size
        )
        _, val_losses = jax.lax.scan(_batch_eval, rng, batched_val_dataset)
        return denoiser_state, ema_denoiser_state, train_losses, jnp.mean(val_losses)

    return train


def train_offline_diffusion(args):
    rng = jax.random.PRNGKey(args.seed)
    local_checkpoint_dir = os.path.join(LOCAL_CHECKPOINT_DIR, "dummy")  # Placeholder

    env, get_bb_cbf_fn, rollout_fn, sample_rollout_fn = None, None, None, None
    # --- Load env functions for losses ---
    if isinstance(args.auxiliary_training_loss, str):
        print(f"Loading environment for {args.auxiliary_training_loss} loss")
        if 'env-sync' in args.auxiliary_training_loss:
            # Load the environment and rollout function
            import test
            # env-sync losses rebuild the evaluation env with a fixed DubinsCar/cover2 config;
            # edit args_str for other envs.
            args_str = ["--path", "pretrained/DubinsCar/gcbf+/", '--area-size', '6', '--num-agents',
                        f"{args.batch_size}",
                        "--nojit-rollout",
                        "--planner", "diffusion", "--spec-len", "30", "--goal-sample-interval", "20",
                        "--spec", "cover2", "--async-planner"]
            test_args = test.test_args(cmd_args=args_str)
            env, get_bb_cbf_fn, rollout_fn, sample_rollout_fn = test.test(test_args, test_debug_rollout=True)

    # --- Construct training and validation datasets ---
    trajs, val_trajs, trajectory_norm_stats, (obs_dim, action_dim) = load_dataset(
        args, normalize=True, val_split=args.val_ratio, unbounded_actions=True
    )
    dataset = stack_transitions(trajs, diffusion_mode=args.diffusion_trajectory_mode)
    val_dataset = stack_transitions(val_trajs, diffusion_mode=args.diffusion_trajectory_mode)
    if args.log:
        wandb.config.update({"norm_stats": trajectory_norm_stats})

    # --- Create denoiser state (and EMA copy) ---
    rng, _rng = jax.random.split(rng)
    denoiser_state = create_denoiser_train_state(
        _rng, obs_dim, action_dim, args, dataset.shape[0]
    )
    ema_denoiser_state = jtu.tree_map(jnp.copy, denoiser_state)

    if args.save_checkpoint:
        # Make sure the directory exists
        # Keeping separate from wandb.run.dir to avoid cluttering the run directory
        if wandb.run is None:
            # If wandb not loaded
            # Create a dummy run directory in local path with current time
            random_run_id = datetime.now().strftime("%Y%m%d-%H%M%S")
            wandb_run_dir_main = f"dummy_run_{random_run_id}"
            os.makedirs(os.path.join(LOCAL_CHECKPOINT_DIR, wandb_run_dir_main), exist_ok=True)
        else:
            wandb_run_dir_main = wandb.run.dir.split('/')[-2]
        # Get absolute path to checkpoint directory
        local_checkpoint_dir = os.path.abspath(os.path.join(LOCAL_CHECKPOINT_DIR, wandb_run_dir_main))
        os.makedirs(local_checkpoint_dir, exist_ok=True)
        options = ocp.CheckpointManagerOptions(max_to_keep=args.checkpoints_to_save, create=True)
        orbax_checkpointer = ocp.PyTreeCheckpointer()
        checkpoint_manager = ocp.CheckpointManager(
            local_checkpoint_dir, orbax_checkpointer, options)

    # --- TRAIN + LOG LOOP ---
    train_fn = jax.jit(make_train(args, dataset, val_dataset, args.eval_rate,
                                  env, rollout_fn, sample_rollout_fn))
    for start_epoch_n in range(1, args.num_epochs + 1, args.eval_rate):
        # --- Train for eval_rate epochs ---
        rng, _rng = jax.random.split(rng)
        denoiser_state, ema_denoiser_state, train_losses, val_loss = train_fn(
            _rng, denoiser_state, ema_denoiser_state
        )

        # --- Log metrics ---
        final_epoch_n = start_epoch_n + args.eval_rate
        print(
            f"Epoch: {final_epoch_n}, Train: {train_losses[-1]:0.4f}, Val: {val_loss:0.4f}"
        )
        if args.log:
            for epoch_idx in range(start_epoch_n, final_epoch_n - 1):
                log(
                    {
                        "epoch": epoch_idx,
                        "train_loss": train_losses[epoch_idx - start_epoch_n],
                    }
                )
            log(
                {
                    "epoch": final_epoch_n,
                    "step": denoiser_state.step,
                    "train_loss": train_losses[-1],
                    "validation_loss": val_loss,
                }
            )

        if args.save_checkpoint and args.checkpoint_every > 0 and final_epoch_n % args.checkpoint_every == 1:
            print(f"Saving step {final_epoch_n} checkpoint to {local_checkpoint_dir}")
            save_args = orbax_utils.save_args_from_target(ema_denoiser_state)
            checkpoint_manager.save(final_epoch_n, ema_denoiser_state,
                                    save_kwargs={'save_args': save_args})

    # --- Save checkpoint ---
    if args.log and args.save_checkpoint:
        print(f"Saving final checkpoint to {os.path.join(wandb.run.dir, CHECKPOINT_DIR)}")
        ckptr = PyTreeCheckpointer()
        ckptr.save(
            os.path.join(wandb.run.dir, CHECKPOINT_DIR),
            ema_denoiser_state,
            save_args=orbax_utils.save_args_from_target(ema_denoiser_state),
        )
        # Push the final checkpoint to wandb, one glob per nesting level
        wandb.save(CHECKPOINT_DIR + "/*")
        wandb.save(CHECKPOINT_DIR + "/*/*")
        wandb.save(CHECKPOINT_DIR + "/*/*/*")


def main(cmd_args=sys.argv[1:]):
    # --- Parse arguments and initialize logging ---
    args = parse_diffusion_args(cmd_args)
    if args.log:
        wandb.init(
            config=args,
            project=args.wandb_project,
            entity=args.wandb_team,
            group=args.wandb_group,
            tags=["unbounded_actions"],
            job_type="train_diffusion",
        )
    debug = args.debug
    debug_nans = args.debug_nans
    if debug_nans:
        jax.config.update("jax_debug_nans", True)

    # --- Launch training ---
    if debug:
        with jax.disable_jit():
            os.environ["WANDB_MODE"] = "disabled"
            os.environ["JAX_DISABLE_JIT"] = "True"
            jax_config.update("jax_disable_jit", True)  # Disable JIT for good measure
            return train_offline_diffusion(args)
    else:
        return train_offline_diffusion(args)


if __name__ == "__main__":
    main()
