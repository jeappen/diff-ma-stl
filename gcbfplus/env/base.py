...
from ..utils.utils import jax2np, jax_vmap, jax_jit_np, tree_concat_at_front, tree_stack
...
class MultiAgentEnv(ABC):
    ...
    def __init__(
        ...
        :param load_map: Optional (map name, fn(key) -> (key, obs_pos, obs_len, obs_theta)) hook for a
                        fixed obstacle map instead of randomly sampled obstacles. No built-in map ships
                        with this release and none of the release commands set it, so it is None
                        everywhere and obstacles are always sampled.
        """
        ...
        # Default Map limits
        self._xy_min = [0, 0]
        self._xy_max = [self._area_size, self._area_size]
        self._load_map = load_map  # Function to load the map if needed

    @property
    def map_name(self) -> str:
        if self._load_map is not None:
            # The first element of the tuple is the map name
            return self._load_map[0]
        return "empty"
    ...
    def rollout_fn(self, policy: Callable, rollout_length: int = None,
                   preprocess_graph: Callable = lambda x: x) -> Callable[[PRNGKey], RolloutResult]:
    ...
    def rollout_fn_jitstep(
            ...
            preprocess_graph: Callable = lambda x: x, end_when_done: bool = False,
            reset_args: Optional[dict] = None
    ):
        """Roll out with a jit-ed step (not a jit-ed loop).

        Signature and 4-tuple return match ``BaseWrapper.rollout_fn_jitstep`` so callers do not
        have to branch on whether the env is wrapped; ``reset_args`` is a wrapper-only hook and
        the bare env has no planner, so its plan info is always empty.
        """
        rollout_length = rollout_length or self.max_episode_steps
    ...
    def process_finished_rollouts(self):
        """Returns function to run on rollout to get any finished rollout metrics"""
        return jax_vmap(self.finish_mask)
