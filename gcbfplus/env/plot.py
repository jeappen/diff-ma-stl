...
def render_video(
        ...
        viz_opts: dict = None,
        dpi: int = 100,
        render_extra: Optional[Callable] = None,
        render_extra_update: Optional[Callable] = None,
        stl_form: STL = None,
        stl_spec_name: str = None,
        key: jax.random.PRNGKey = None,
        **kwargs
):
    """Render a video of the rollout.

    Note: the pgf snapshot title prints one STL formula; per-agent specs (mixed/team) show
    only stl_form.

    :param rollout: RolloutResult
    :param render_extra: To render extra fixed objects. Args: ax, rollout, viz_opts
    :param render_extra_update: To render extra objects that change over time. Args: ax, rollout, viz_opts, kk, safe_text
                                safe_text is to update the text in the video.
    """
    ...
    if dim == 2:
        ...
        agent_col_positions = MutablePatchCollection([i for i in reversed(agent_circs[:n_agent])], match_original=True,
                                                     zorder=6)
        agent_col_goal = MutablePatchCollection([i for i in reversed(agent_circs[n_agent:])], match_original=True,
                                                zorder=6)
        ax.add_collection(agent_col_positions)
        ax.add_collection(agent_col_goal)
        agent_col = agent_col_positions
    else:
        plot_r = ax.transData.transform([r, 0])[0] - ax.transData.transform([0, 0])[0]
        agent_col = ax.scatter(n_pos[:, 0], n_pos[:, 1], n_pos[:, 2],
                               s=plot_r, c=n_color, zorder=5)  # NOTE: 3-D marker size is approximate.

    if render_extra is not None:
        # Runs any extra static renderings such as STL predicates.
        render_extra(ax, rollout, viz_opts, key=key)
    ...
    if Ta_is_unsafe is not None:
        if dim == 2:
            safe_text = [ax.text(0.02, 0.98, "Unsafe: {}", va="bottom", **text_font_opts)]
        else:
            safe_text = [ax.text2D(0.02, 0.98, "Unsafe: {}", va="bottom", **text_font_opts)]
    ...
    def update(kk: int) -> list[plt.Artist]:
        ...
        if render_extra_update is not None:
            # Runs any extra dynamic renderings such as current time step.
            render_extra_update(ax, rollout, viz_opts, kk, safe_text)

        return [agent_col, edge_col, *agent_labels, cost_text, *safe_text, *cnt_col_t, kk_text]

    anim_T = len(T_graph.n_node)
    if not viz_opts.get('plot_only_img', False):
        # Save the video
        speed_factor = viz_opts.get('speed_factor', 1.0)
        fps = 30.0 * speed_factor
        spf = 1 / fps
        mspf = 1_000 * spf

        # Add highlighted agent circles with light black borders
        if 'highlight_agents' in viz_opts:
            highlighted_agents = viz_opts['highlight_agents']
            if dim == 2:
                highlight_circs = [plt.Circle(n_pos[ii], n_radius[ii] * 3.0, color='k',
                                              linewidth=2.0, fill=False, alpha=0.5, zorder=5)
                                   for ii in highlighted_agents if ii < n_agent]
                highlight_col = MutablePatchCollection(highlight_circs, match_original=True, zorder=5)
                ax.add_collection(highlight_col)

                # Add to the update function's return list
                init_return_list = init_fn()
                old_update = update

                def new_update(kk):
                    result = old_update(kk)
                    graph = tree_index(T_graph, kk)
                    n_pos_t = graph.states[:-1, :dim]

                    # Update highlight circles
                    for i, agent_idx in enumerate(highlighted_agents):
                        if agent_idx < n_agent:
                            highlight_circs[i].set_center(tuple(n_pos_t[agent_idx]))

                    return result + [highlight_col]

                update = new_update
                init_fn = lambda: init_return_list + [highlight_col]
    ...
    if viz_opts.get('plot_snapshot', False):
        if viz_opts.get('plot_pgf', False):
            matplotlib.use("pgf")
            matplotlib.rcParams.update({
                "pgf.texsystem": "pdflatex",
                'pgf.preamble': "\\usepackage{latexsym}",
                'font.family': 'serif',
                'text.usetex': True,
                'pgf.rcfonts': False,
            })
            ...
            # Show stl form on top left
            if stl_form is not None and stl_spec_name is not None:
                stl_text = ax.text(-0.11, 1.09, f"{stl_spec_name} Spec.: ${stl_form.latex_repr()}$", va="top",
                                   ha="left",
                                   color="k", fontsize=42, transform=ax.transAxes)
        # Remove text
        cost_text.remove()
        for st in safe_text:
            st.remove()
        kk_text.remove()
        # Remove goal circle and lines
        if dim == 2:
            agent_col_goal.remove()
        edge_col.remove()

        # Faded initial starting points
        if dim == 2:
            if viz_opts.get('plot_only_img', False):
                # Move the darkened circles to the final position

                graph = tree_index(T_graph, anim_T - 1)  # Get the last graph
                n_pos_t = graph.states[:-1, :dim]
                ...
                for ii in range(n_agent):
                    agent_circs[ii].set_center(tuple(n_pos_t[ii]))
                    agent_labels[ii].set_position(n_pos_t[ii])
            # Faded circles mark initial positions; with plot_only_img the solid circles
            # were moved to the final frame above.
            agent_circs_faded = [plt.Circle(n_pos[ii], n_radius[ii], color=n_color[ii], linewidth=0.0, alpha=0.2,
                                            label="Initial Position" if ii == 0 else None)
                                 for ii in range(n_agent)]
            agent_col = MutablePatchCollection([i for i in reversed(agent_circs_faded)], match_original=True,
                                               zorder=5)  # Behind current positions
        ...
        if dim == 2:
            agent_labels = [ax.text(n_pos[ii, 0], n_pos[ii, 1], f"{ii}", alpha=0.2, **label_font_opts) for ii in
                            range(n_agent)]
        else:
            for ii in range(n_agent):
                pos2d = proj3d.proj_transform(n_pos[ii, 0], n_pos[ii, 1], n_pos[ii, 2], ax.get_proj())[:2]
        ...
        # Set label for agent_circs
        agent_circs[0].set(label="Final Position")

        # Add patches to the legend
        class HandlerCircle(HandlerPatch):
            """To handle the legend for the circle patches."""

            def create_artists(self, legend, orig_handle,
                               xdescent, ydescent, width, height, fontsize, trans):
                center = 0.5 * width - 0.5 * xdescent, 0.5 * height - 0.5 * ydescent
                p = plt.Circle(xy=center, radius=6)
                self.update_prop(p, orig_handle, legend)
                p.set_transform(trans)
                return [p]

        if viz_opts.get("plot_plan_and_quit", False):
            # Only show initial position if not plotting the plan and quitting with no rollout
            agent_circs[0].set(label="Initial Position")
            handles = [agent_circs[0]]
            plt.axis('off')
        else:
            handles = [agent_circs[0], agent_circs_faded[0]]
        ax.legend(handles=handles, loc='upper right', fontsize=16,
                  handler_map={plt.Circle: HandlerCircle()})

        # Save the snapshot with new filename
        if viz_opts.get('plot_pgf', False):
            snapshot_path = video_path.with_suffix(".pgf")
        elif viz_opts.get('plot_svg', False):
            snapshot_path = video_path.with_suffix(".svg")
        else:
            snapshot_path = video_path.with_suffix(".png")  # Save as png for general use

        # savefig fig and ax
        print(f"Saving snapshot to {snapshot_path}")
        fig.tight_layout()
        fig.savefig(snapshot_path, bbox_inches='tight', dpi=300)
