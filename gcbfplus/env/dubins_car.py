...
from ..utils.typing import Action, AgentState, Array, Cost, Done, Info, Reward, State, PRNGKey
...
class DubinsCar(MultiAgentEnv):
    ...
    def __init__(
        ...
        super(DubinsCar, self).__init__(num_agents, area_size, max_step, max_travel, dt, params, load_map)
    ...
    def stay_stationary(self, agent_states: AgentState, action: Action,
        ...
        def update_element(action_i, agent_state_i, mask_i):
            ...
            updated_action_i = jax.lax.cond(
                mask_i,
                lambda _: jax.lax.concatenate((jnp.zeros(1), agent_state_i[-1:] * -1.0), dimension=0),
                lambda _: action_i,
                operand=None
            )
    ...
    def render_video(
            self, rollout: RolloutResult, video_path: pathlib.Path, Ta_is_unsafe=None, viz_opts: dict = None,
            dpi: int = 80, **kwargs
    ) -> None:
        render_video(
            rollout=rollout,
    ...
    def edge_blocks(self, state: EnvState, lidar_data: State) -> list[EdgeBlock]:
        ...
        for i in range(self.num_agents):
            agent_goal_edges.append(
                EdgeBlock(agent_goal_edge_feats[i][None, None, :], jnp.ones((1, 1)), id_agent[i][None],
                          id_goal[i][None]))
