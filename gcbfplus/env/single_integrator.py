...
from .obstacle import Rectangle, Obstacle
...
class SingleIntegrator(MultiAgentEnv):
    ...
    def __init__(
        ...
        super(SingleIntegrator, self).__init__(num_agents, area_size, max_step, max_travel, dt, params, load_map)
    ...
    def goal_dim(self) -> int:
        """Dimension of the goal space 2D or 3D"""
        return 2
    ...
    def reset(self, key: Array) -> GraphsTuple:
        self._t = 0
        key, obstacles = self.create_all_obstacles(key)
    ...
    def create_all_obstacles(self, key: PRNGKey) -> Tuple[PRNGKey, Obstacle]:
        if self._load_map is not None:
            # Load the fixed map if needed
            key, obs_pos, obs_len, obs_theta = self._load_map[1](key)
            obstacles = self.create_obstacles(obs_pos, obs_len[:, 0], obs_len[:, 1], obs_theta)
            return key, obstacles
        else:
            return self.create_random_obs(key)

    def create_random_obs(self, key):
    ...
    def stay_stationary(self, agent_states: AgentState, action: Action,
                        unsafe_mask: Array) -> Action:
        """Return the action to stay stationary. Used for Prioritize Safety tests."""

        def update_element(action_i, agent_state_i, mask_i):
            # Using lax.cond to apply the update only if mask_i is True
            # set action to 0 if unsafe for that agent
            updated_action_i = jax.lax.cond(
                mask_i,
                lambda _: jnp.zeros(self.action_dim),
                lambda _: action_i,
                operand=None
            )
            return updated_action_i

        # Using vmap to vectorize the update_element function
        stationary_action = jax.vmap(update_element)(action, agent_states, unsafe_mask)
        return stationary_action
    ...
    def step(
            self, graph: MultiAgentEnv.EnvGraphsTuple, action: Action, get_eval_info: bool = False
    ) -> Tuple[MultiAgentEnv.EnvGraphsTuple, Reward, Cost, Done, Info]:
        self._t += 1

        # calculate next graph
        agent_states = graph.type_states(type_idx=self.AGENT, n_type=self.num_agents)
        goals = graph.type_states(type_idx=self.GOAL, n_type=self.num_agents)
        ...
        # the episode ends when reaching max_episde_steps
    ...
    def edge_blocks(self, state: MultiAgentEnv.EnvState, lidar_data: Pos2d) -> list[EdgeBlock]:
    ...
    def get_graph(self, state: MultiAgentEnv.EnvState) -> GraphsTuple:
        # node features
        n_hits = self._params["n_rays"] * self.num_agents
    ...
    def finish_mask(self, graph: GraphsTuple) -> Array:
        """Whether an agent has finished its task."""
