...
COMMENT_SEPARATOR = ";"
FLOAT_FORMAT = "4.4f"  # 4 decimal places for info metrics
FLOAT_FORMAT_LESS = "4.2f"  # For STL metrics with less precision required
...
class Empty:
    """Empty class to be used as a placeholder for None values."""
    pass


def turn_dict_into_comment_str(args_dict, args_of_interest):
    """Convert a dictionary of arguments into a comment string."""
    parts = []
    for key in args_of_interest:
        ...
        if value is not None:
            if isinstance(value, bool):
                # For boolean flags, include the flag only if True
                if value:
...
def format_for_wandb(dict_to_format):
    """Format dictionary for wandb logging"""

    def _format_value(v):
        # Convert to int, float, bool if possible
        if isinstance(v, str):
            if v.isdigit():
                return int(v)
            elif '.' in v:
    ...
    return {k: _format_value(v) for k, v in dict_to_format.items()}


def get_git_diff():
    """Get the git diff and config diff to be added to the experiment log."""
    try:
        diff = subprocess.check_output(["git", "diff"]).decode("utf-8")
    except Exception as e:
        diff = f"Error collecting git diff: {e}"

    try:
        # Only diff the config file
        config_diff = subprocess.check_output(
            ["git", "diff", "--unified=0", "gcbfplus/utils/configs/default_config.yaml"]).decode("utf-8")
    except Exception as e:
        config_diff = f"Error collecting config diff: {e}"
    return diff, config_diff
...
def jax_jit_np(fn: _Fn, static_argnums: int | Sequence[int] | None = None,
               static_argnames: str | Iterable[str] | None = None, donate_argnums: int | Sequence[int] = (),
               device: xc.Device = None, *args, **kwargs, ) -> _Fn:
...
class MultipleLossTrainState(struct.PyTreeNode):
        ...
        for i, grads_for_opt in grads_with_opt.items():
            ...
            def get_switch_callable(i):
                ...
                def update_op(opt_states, params, grads):
                    tx = self.tx[i]
                    updates, new_opt_state = tx.update(grads, opt_states[i], params)
            ...
            params_with_opt, new_opt_states = jax.lax.switch(i, [get_switch_callable(_i) for _i in range(num_losses)],
                                                             new_opt_states, params_with_opt,
                                                             grads_for_opt)  # As implied by the OWG name, the gradients are used directly to update the
        # parameters.
        if OVERWRITE_WITH_GRADIENT in grads:
            # TODO: May not work with multiple losses
            new_params = {'params': params_with_opt, OVERWRITE_WITH_GRADIENT: grads[OVERWRITE_WITH_GRADIENT], }
        else:
            new_params = params_with_opt
        return self.replace(step=self.step + 1, params=new_params, opt_states=new_opt_states, **kwargs, )
    ...
    def create(cls, *, apply_fn, params, tx: List, **kwargs):
        ...
        return cls(step=0, apply_fn=apply_fn, params=params, tx=tx, opt_states=opt_states, **kwargs, )
