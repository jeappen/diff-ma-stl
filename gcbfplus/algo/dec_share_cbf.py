...
import gcbfplus.env.wrapper as env_wrapper


class DecShareCBF(MultiAgentController):
    """Decentralized CBF-QP over the k closest agents/obstacles.

    Each agent solves its own QP against its k nearest neighbours, taking full
    responsibility (1.0) for agent-obstacle constraints and a shared half (0.5) for
    agent-agent pairs, so both agents in a pair together cover the constraint.
    """
    ...
    def __init__(
        ...
        self.k = 3
        if isinstance(env, env_wrapper.BaseWrapper):
            env = env.env
