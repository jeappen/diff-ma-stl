...
def get_pwise_cbf_fn(env: MultiAgentEnv, k: int = 3):
    ...
    # NOTE: pwise_cbf_cfhl above is an alternative CrazyFlie pairwise CBF; it had a second
    # `isinstance(env, CrazyFlie)` branch here that the branch above always shadowed, so it
    # was unreachable and has been removed. Dispatch to it explicitly if you want it.
