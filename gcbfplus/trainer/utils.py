...
if TYPE_CHECKING:
    ...
    from ..env.wrapper import PlannerWrapper
else:
    MultiAgentEnv = None
    PlannerWrapper = None
...
def rollout(
        env: Union[MultiAgentEnv, PlannerWrapper],
        actor: Callable,
        key: PRNGKey,
        preprocess_graph: Callable = lambda x: x,
        init_graph: GraphsTuple = None,
        max_length: int = None,
    ...
    key: PRNGKey
    preprocess_graph: Callable, GraphsTuple -> GraphsTuple
    init_graph: GraphsTuple
    max_length: int
    single_goal: bool, whether to do a rollout till 1 goal_sample_interval
    ...
    if max_length is None:
        if single_goal:
            max_length = env.goal_sample_interval
        else:
            max_length = env.max_episode_steps
    ...
    final_graph, (graphs, actions, rewards, costs, dones, log_pis, next_graphs) = \
        jax.lax.scan(body, init_graph, keys, length=max_length)
...
def is_connected():
    """Probe for internet access; callers use it to pick wandb's online vs offline mode."""
    try:
