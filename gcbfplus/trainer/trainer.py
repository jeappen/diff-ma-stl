...
from ..env.wrapper import STLWrapper
...
class Trainer:
    ...
    def __init__(
        ...
        if os.environ.get("WANDB_MODE", "online") not in ("disabled", "offline"):
            wandb.login()
        wandb.init(name=params['run_name'],
                   project='my-project',  # EDIT: your wandb project name
                   dir=self.log_dir)
    ...
    def config(self) -> dict:
        return self.params | {'exp_log_dir': self.log_dir}
    ...
    def train(self):
        ...
        def rollout_fn_single(params, key):
            return rollout(self.env, ft.partial(self.algo.step, **params), key,
                           preprocess_graph=ft.partial(self.algo.preprocess_graph, **params))
        ...
        for step in range(0, self.steps + 1):
            ...
            if step % self.eval_interval == 0:
                test_rollouts: Rollout = test_fn(self.algo.algo_params, test_keys)
                ...
                finish_fun = jax_vmap(self.env_test.process_finished_rollouts())
                finish_info = {}  # For any other metrics during evaluation
                finish_info_str = ''
                if isinstance(self.env_test, STLWrapper):
                    # Now finish has STL related metrics
                    finish = finish_fun(test_rollouts.graph).mean()
                    finish_metrics = jax_vmap(self.env_test.process_finished_rollouts_info())(test_rollouts.graph)
                    finish_info = {f"eval/{k}": v.mean() for k, v in finish_metrics.items()}
                    finish_info_str = ', ' + ', '.join([f'{k}: {v.mean() :4.2f}' for k, v in finish_metrics.items()])
                else:
            ...
            rollouts: Rollout = rollout_fn(self.algo.algo_params, key_x0)
